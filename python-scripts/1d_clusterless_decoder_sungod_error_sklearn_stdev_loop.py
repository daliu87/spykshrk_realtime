...
# this version runs a 5-fold cross-validation for 1 epoch using sklearn and a giant for loop :o
...
#read in start times and end times from trials.mat
...
# for loop for differnt mark kernel stdev
#mark_kernel_stdev = [5,10,20,40,80]
pos_kernel_stdev = [0.25, 0.5, 2., 4.]
#pos_kernel_stdev = [1]
for outer_loop_counter in range(len(pos_kernel_stdev)):
    print('Position kernel stdev = ',pos_kernel_stdev[outer_loop_counter])
    print('Position kernel stdev = ',pos_kernel_stdev[outer_loop_counter],file=open("/data2/mcoulter/1d_decoder_log.txt","a"))

    # define 80/20 splits with Kfold
    # setup for loop to run encoder and decoder on each 80/20 split

    crossval_counter = 1
    kf = KFold(n_splits=5, shuffle=True)
    for encode, decode in kf.split(starttimes):
        print("%s %s" % (encode, decode))
        print("%s %s" % (starttimes[encode], starttimes[decode]))
        print("%s %s" % (encode, decode), file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        print("%s %s" % (starttimes[encode], starttimes[decode]), file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        ...
        print('Running subset ',crossval_counter,' of 5 for cross validation.')
        print('Running subset ',crossval_counter,' of 5 for cross validation.', file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        ...
        encode_settings = AttrDict({'sampling_rate': 3e4,
                                    ...
                                    'pos_kernel_std': pos_kernel_stdev[outer_loop_counter], 
                                    'mark_kernel_std': 10, 
        ...
        #print('Mark kernel stdev: ',mark_kernel_stdev)
        #print('Mark kernel stdev: ',mark_kernel_stdev, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        ...
        # for loop to make encoding and decoding subsets
        #encoding position
        ...
        print('Encoding position: ',encode_trial_pos_all.shape)
        print('Encoding position: ',encode_trial_pos_all.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        ...
        print('Encoding marks: ',encode_trial_marks_all.shape)
        print('Encoding marks: ',encode_trial_marks_all.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        ...
        print('Decoding marks: ',decode_trial_marks_all.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))

        # filter for large negative marks
        encode_trial_marks_all_non_negative = trodes2SS.threshold_marks_negative(encode_trial_marks_all, negthresh=-999)
        decode_trial_marks_all_non_negative = trodes2SS.threshold_marks_negative(decode_trial_marks_all, negthresh=-999)        
        print('Original encode length: ',encode_trial_marks_all.shape)
        print('Original encode length: ',encode_trial_marks_all.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        print('Encoding marks non-negative filter: ',encode_trial_marks_all_non_negative.shape)
        print('Encoding marks non-negative filter: ',encode_trial_marks_all_non_negative.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        print('Original decode length: ',encode_trial_marks_all.shape)
        print('Original decode length: ',encode_trial_marks_all.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        print('Decoding marks non-negative filter: ',encode_trial_marks_all_non_negative.shape)
        print('Decoding marks non-negative filter: ',encode_trial_marks_all_non_negative.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))

        # filter for mark amplitude
        encode_trial_marks_all_sparse = trodes2SS.threshold_marks(encode_trial_marks_all_non_negative, maxthresh=2000,minthresh=100)
        decode_trial_marks_all_sparse = trodes2SS.threshold_marks(decode_trial_marks_all_non_negative, maxthresh=2000,minthresh=100)
        print('Original encode length: ',encode_trial_marks_all_non_negative.shape)
        print('Original encode length: ',encode_trial_marks_all_non_negative.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        print('Encoding marks amplitude filter: ',encode_trial_marks_all_sparse.shape)
        print('Encoding marks amplitude filter: ',encode_trial_marks_all_sparse.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        print('Original decode length: ',decode_trial_marks_all_non_negative.shape)
        print('Original decode length: ',decode_trial_marks_all_non_negative.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        ...
        print('Encoding position velocity filter: ',pos_encode_crossval.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        ...
        print('Encoding marks velocity filter: ',spk_subset_encode_crossval.shape)
        ...
        print('Decoding marks velocity filter: ',spk_subset_decode_crossval.shape)
        print('Decoding marks velocity filter: ',spk_subset_decode_crossval.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        ...
        encoder = OfflinePPEncoder(linflat=pos_encode_crossval, dec_spk_amp=spk_subset_decode_crossval, encode_settings=encode_settings, 
        ...
        print('observations object: ',observ_obj.shape)
        ...
        #cell 15.1
        # filter posterior dataframe so it only includes decoded times
        ...
        print('Filtered posteriors: ',posteriors_filtered.shape)
        print('Filtered posteriors: ',posteriors_filtered.shape, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))    

        #cell 16
        #save posteriors with hdf
        #post_hdf_filename = '/data2/mcoulter/posteriors/remy_20_2_crossval_new_' + str(crossval_counter) + '_mark_stdev_20_posterior.h5'
        #posteriors._to_hdf_store(post_hdf_filename,'/analysis', 
        #                         'decode/clusterless/offline/posterior', 'learned_trans_mat')
        #print('Saved posteriors to ',post_hdf_filename)
        #print('Saved posteriors to ',post_hdf_filename, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        ...
        post_netcdf_filename = '/data2/mcoulter/posteriors/gus_28_2_crossval_new_mark_10_' + str(crossval_counter) + '_pos_stdev_' + str(pos_kernel_stdev[outer_loop_counter]) + '_posterior.nc'
        ...
        post3.to_netcdf(post_netcdf_filename)
        print('Saved posteriors to ',post_netcdf_filename)
        ...
        position_netcdf_filename = '/data2/mcoulter/linearized_position/gus_28_2_crossval_new_mark_10_' + str(crossval_counter) + '_pos_stdev_' + str(pos_kernel_stdev[outer_loop_counter]) + '_linear_position.nc'
        ...
        linearized_pos3.to_netcdf(position_netcdf_filename)
        print('Saved linearized position to ',position_netcdf_filename)
        print('Saved linearized position to ',position_netcdf_filename, file=open("/data2/mcoulter/1d_decoder_log.txt","a"))

        print("End of script!")
        print("End of script!", file=open("/data2/mcoulter/1d_decoder_log.txt","a"))
        print(" ", file=open("/data2/mcoulter/1d_decoder_log.txt","a"))

        # insert end of for loop here
        # only thing needed is i = i+1 for saving filename
        crossval_counter = crossval_counter+1
